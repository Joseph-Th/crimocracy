...
- **Governed rival world** — the Rosetti organization expands home rackets identically across matched branches unless branch actions heat the shared district (police fear throttles expansion while it decays); delegated growth considers all viable authored kinds, refuses non-positive current zero-variance net, preserves explicit neighborhood/business authority priority, honors organization-wide `Function(Enterprise)` as a broad fallback, and chooses the strongest current economics inside a tier with stable deterministic tie-breakers; observed audit-only via `rival_home_enterprises`.
