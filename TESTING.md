...
- Policy-seed-derived variation inside fixed branch identities: the witness-pressure delay used only when no actionable patrol pattern is known, and which rival the defector watch visits first. The win-back pitch is not policy-seed variation: leadership matches the approach to the candidate's organization-visible drives and traits (Protection for a frightened Safety-driven member, FinancialOpportunity for a Money-driven candidate), because random pitches would manufacture failure instead of playing the recruitment decision.
...
- **Rival intelligence** — after a normal daily rival-expansion boundary, watch a known rival organization, select the first player-held Personnel observation with an Enterprise subject, and attach it to one direct follow-up watch. No hidden enterprise enumeration supplies targets. Failed/partial discovery records absence rather than inventing a target; the default-world regression requires the complete discovery/follow-up chain. The focused enterprise watch additionally learns local PoliceActivity, with typed patrol intervals only on an achieved watch; broad organization discovery does not reveal patrols at every racket location. `rival-intelligence-w<world>-p<policy>.json` retains exact summaries, quality, typed signals, observation times, source identities, exposure and attached information; artifact IO failures propagate. Existing defector watches also surface their incidental racket sightings in the narrative, closing view and per-run `player_visible.known_rackets`; these are historical, bounded observations, never live portfolio totals. Production regressions cover bounded active-only discovery, partial/failed withholding, exact-subject authorization, snapshot freshness and persistence. Observations and after-action findings name the observed racket kind so several activities at one venue remain distinguishable without disclosing revenue or cases; a co-located three-kind regression also preserves those summaries across save/restore.
...
- **Manager reports reach leadership** — notable enterprise settlements atomically persist a Financial report citing the manager's organization-held information, so executive briefs surface the same observed economics, changed street surcharge, vice warning, and loss suspension without revealing formal case internals. Each settlement summary names the racket kind, location, and responsible manager; district- and business-hosted regressions protect attribution and exact source-summary preservation in the brief. Unchanged profitable, low-variance heat settles as routine without another report. Focused regressions cover brief inclusion without pending decisions, unchanged-heat silence, and report-ID exhaustion rejecting the entire settlement without mutation. Narration quotes only this organization's cycles; live rival racket totals remain diagnostic-only.
