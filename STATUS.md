...
| Operations | Semantic plans, objectives, approaches, roles, participant reservations, intelligence, timing, contingencies, deterministic outcomes, exposure, surveillance, police response, after-action records, property disposition | `src/operations/` via `operation_system` and `operation_execution` |
...
| Enterprises | Routine criminal enterprises, authored economics, manager authority, scheduled cycles, balanced settlement, financial reporting | `src/enterprises/` via `enterprise_execution` and reporting modules |
