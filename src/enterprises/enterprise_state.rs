//! Durable enterprise state, derived indexes, and synchronized internal mutations.

use super::{EnterpriseCycleRecord, EnterpriseLocation, EnterpriseRecord, EnterpriseStatus};
use crate::core::id::{
    BusinessId, EnterpriseCycleId, EnterpriseId, FinancialAccountId, IdKeyedBounds, MandateId,
    OrganizationId,
};
...
impl EnterpriseState {
    ...
    pub(super) fn insert(&mut self, record: EnterpriseRecord) {
        let id = record.id();
        self.by_organization
            .entry(record.organization())
            .or_default()
