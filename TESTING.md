...
| Lib tests (no soak) | `cargo test-fast` | ~0.11s | ~12s | 371 lib tests, `--skip soak` |
...
- **Legal foundation** — arrest → paid counsel → custody-preserving prosecution referral → terminal decline → named cooperative witness intimidated through canonical pressure with mandatory cooperation degradation. Focused legal tests additionally prove custody cannot be evaded by internal operation/work bookings, detained investigators release their case seats, conflicted subjects cannot investigate themselves, detained prosecutors release reviews for deterministic office restaffing without rewriting historical referral actors, and direct/automatic counsel can aggregate sponsor liquidity while delegated counsel remains confined to its mandate budget account.
