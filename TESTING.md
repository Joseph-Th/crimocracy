...
substring `soak`. Scenario-scale harness tests are explicitly `#[ignore]` with a reason that
names `cargo test-harness-deep`; that alias runs every ignored harness test, so a function rename
cannot silently drop deep coverage. These deeper tiers are evidence, not prerequisites for unrelated edits.
...
|---|---|---|
| Docs/routes only | `.\scripts\check-docs.cmd` | authorities, links, routes, aliases, published versions; no Rust build |
...
| `full` | `cargo harness-full --samples 8` | Concise strategy/probe summaries, matched world/policy seed channels, scenario sensitivity, artifacts |
| `full detail` | `cargo harness-full-detail --samples 8` | Same gameplay evidence plus the primary player-facing narrative/deep metric readout |

Commands: `cargo harness` runs smoke by default on the normal dev cache; `cargo harness-full --samples 8` runs the explicit optimized deep profile; append `--artifact-dir target/my-run` to relocate artifacts. `cargo harness-full-detail` changes presentation only, not policy or simulation behavior. `cargo harness -- --mode smoke --strategy press` selects one branch.
...
`--world-seed` controls fixture variation and the simulation RNG streams. `--policy-seed` independently controls evaluation-owned timing and bounded policy choices. In smoke mode those flags are the exact seed pair. In full mode they are the base pair for a deterministic sample schedule: `--samples N` (range 1..=64) varies both streams independently without increasing the number of sessions, and all strategy branches inside one sample receive the exact same pair. Focused contract tests still hold one axis fixed when causal isolation matters. Per-run events and `RunMetrics` are raw evidence beneath aggregates; aggregates are not quality scores.

- `full` mode writes batch per-run JSON to `--artifact-dir` (default `target/harness-runs/`), all rotated full-session runs to its `narrative/` subdirectory (never overwriting same-seed batches), plus `summary-w<world>-p<policy>.json`. Each run artifact records both seed channels and separates `player_visible` evidence from `diagnostic` hidden-state evidence. Opening-score artifacts also retain the player-visible decision budget: current minute, opportunity lifetime, authored burglary/surveillance durations, scout-time slack, the selected posture, and the live posture alternatives. Evaluators should not have to reverse-engineer whether information was cheap, costly, or already too late.
...
- Full mode rotates its narrative comparison across `NARRATIVE_SEED_ROTATION` reproducible world/policy seed pairs. The world stream covers every authored fixture variation (economy profile, police presence, patrol windows, target names, till kind), while the policy stream slightly varies evaluation-owned choices without consuming simulation RNG. Every set validates deterministic contracts plus any stochastic consequence that actually occurs, but player follow-up is required only when player-visible evidence crossed the acting-policy boundary: a hidden trace-level institutional case remains diagnostic evidence and must not manufacture a contact query or counter-surveillance action. Stochastic reachability contracts are evaluated across sufficiently broad paired-seed batches rather than forced in every sample. Detailed mode prints the deep metrics and experience readout for the primary base pair; concise mode prints compact summaries for every rotated set. Both execute the same validation contracts.
- Authored-content-derived timing: scenario anchors use the canonical campaign-day length and authored operation durations; legal recovery waits use the production cold-case window, and the terminal-wait guard's slack equals the longest authored operation duration. Recruitment cadence is not treated as a proxy for the length of a campaign day.
- Policy-seed-derived variation inside fixed branch identities: the witness-pressure delay used only when no actionable patrol pattern is known, which rival the defector watch visits first, and RECON's conservative 45/60/75-minute uncertainty margin around player-visible patrol concentrations. The chosen patrol margin is persisted in run artifacts. The win-back pitch is not policy-seed variation: after surveillance confirms the defector's destination, leadership makes one PersonalAppeal grounded in the already-authored boss-member relationship. The harness does not inspect latent candidate drives or traits to choose that pitch; production recruitment scoring still uses the candidate's private willingness factors to determine whether the appeal succeeds.
...
- Scenario-sensitivity profiles isolate different decision pressures rather than serving as cosmetic variants: `LATE PATROL` keeps immediate action viable while learned patrol timing remains useful; explicit patrol windows are concentrations, and off-window policing retains the authored residual share of ambient district presence, so a bounded batch may legitimately observe zero or several residual police arrivals. `FLEETING WINDOW` makes reconnaissance consume a genuinely closing opportunity so informed stand-down is a valid timing outcome rather than a harness error; `VETERAN CREW` raises execution capability without erasing institutional consequences; `THIN CREW` proves information cannot substitute for missing overall crew capability; `GREEN SCOUT` leaves the burglary crew, opportunity window, and police environment at baseline while lowering only the scout, proving that reconnaissance is itself risky field work whose usefulness depends on personnel capability rather than an automatic dominant prerequisite. Once the batch spans the authored fixture variations, the named controls carry explicit harness contracts so later tuning cannot silently erase the causal contrast their labels promise.
...
- **Own-heat loop (RECON)** — the second scout reuses the newest organization-held typed patrol observation for its district, with the policy clock as an earliest readiness time and a conservative policy-seeded 45/60/75-minute uncertainty margin around patrol intervals. The selected patrol report is attached to the scout's canonical plan as well as used to select its schedule; contract and artifact evidence retain both attachment and production-scored topic coverage. The narrative and artifact retain the chosen margin, observation age, and actual canonical scout schedule; a pinned contract protects the primary seed's clean second score rather than forcing exposure for coverage. Known windows protect looking as well as taking, without promising safety. The trigger for a case query is player-visible crew exposure, not hidden intake: when fresh casing resolves with any observed exposure, the branch checks that operation's case through its standing institutional contact before authorizing another burglary. A confirmed active case or an inconclusive channel read makes RECON stand down and let the reopened score expire; only an explicit shelved read (or a casing run with no observed exposure) permits a lower-risk burglary timed outside the known patrol concentrations. A session whose casing reported no exposure must not fabricate a read. Full mode also runs a controlled GREEN SCOUT / CROWDED probe that selects the authored pressure fixture, not a favorable hidden outcome: the scout must return useful findings, trigger a real contact read, receive an active-case disclosure, and withhold the burglary. This keeps the self-heat experience visible even when the primary narrative happens to case cleanly.
- **Witness chain and counter-play** — the target is character-owned, so witnessed/identifying exposure names the owner as on-scene witness through canonical intake; institutional interviews can convert his account into testimony; PRESS answers with exactly one WitnessPressure operation. When organization-held typed patrol information exists it is binding and must yield a lower-risk pre-interview window outside known patrol concentrations; if it does not, the treatment fails rather than discarding known risk. Only when no actionable patrol pattern is known may a bounded policy-seed delay be used, disclosed as treatment timing rather than invented patrol knowledge. Two honest terminal shapes: registered cooperation degrades, or a police response forces a disciplined abort that leaves no second case. Failed pressure without degradation fails the run. Identifying exposures can escalate to autonomous member arrests (`player_member_arrests`); acting policy never reads case internals.
...
- **Opportunity prioritization** — under the `LATE PATROL` low-immediate-pressure control, two burglary opportunities share a closing window and one entry specialist. Isolating the probe from an early police abort matters: if a first job released the specialist early, the other score would be live again and the harness could not honestly call it a forced tradeoff. The harness chooses the stronger player-visible source at a time when either score could start, then the selected burglary keeps that specialist occupied beyond the shared deadline, so the weaker score expires as a real time/capacity cost rather than arbitrary inaction. The expired opportunity keeps its report; a later decoy dismisses through the canonical lifecycle.
- **Rival intelligence** — after a normal daily rival-expansion boundary, watch a known rival organization, select the first player-held Personnel observation with an Enterprise subject, and attach it to one direct follow-up watch. No hidden enterprise enumeration supplies targets; failed/partial discovery records absence rather than inventing a target. The default-world contract requires the complete discovery/follow-up/intervention chain.
  - Enterprise observations persist a typed exact operating location (`Business` or `Neighborhood`) alongside their Enterprise subject. That typed fact is the only source used to choose the rival venue for later action; the harness never parses the display summary or consults the live rival enterprise record to recover location.
  - The focused enterprise watch additionally learns local PoliceActivity, with typed patrol intervals only on an achieved watch; broad organization discovery does not reveal patrols at every racket location. The enterprise-location fact chooses the venue but is not smuggled into business-target operation scoring; only player-held information relevant to the sabotage objective is attached to the plan. The intervention also requires `PoliceActivity` as a real operation constraint and carries `AbortOnPoliceArrivalBeforeEntry`, so the learned local pattern is a production authorization prerequisite and the demonstration retains a standing safety response instead of treating surveillance as decorative.
  - The probe then authorizes canonical covert sabotage against the learned venue and requires a real production consequence. A cloned matched continuation with no intervention is evaluation-only: it never feeds acting policy, and the contract requires the sabotaged venue's next business cycle to show a material gross and net reduction versus that same-state counterfactual.
  - `rival-intelligence-w<world>-p<policy>.json` keeps player-visible discovery, typed location, follow-up, intervention, and after-action evidence separate from matched counterfactual economics under explicit evaluation data; artifact IO failures propagate.
  - Existing defector watches also surface their incidental racket sightings in the narrative, closing view and per-run `player_visible.known_rackets`; typed locations are rendered as actionable venues/districts while remaining historical, bounded observations rather than live portfolio totals.
  - Production contract tests cover bounded active-only discovery, partial/failed withholding, exact-subject authorization, typed location compatibility/reference integrity, snapshot freshness and persistence. Observations and after-action findings name the observed racket kind so several activities at one venue remain distinguishable without disclosing revenue or cases; a co-located three-kind contract test also preserves those summaries across save/restore.
- **Violence / reputation leverage** — matched branches run the same first arson with covert versus visible violence, then the same prepared, time-pressed intimidation against another business. The first violent act must pay its normal exposure/police cost while moving business-owner fear; no exact standing score informs acting policy. The follow-up is deliberately placed on the production achieved threshold so the existing bounded one-point fear adjustment changes a Partial collection into Achieved and materially changes cash proceeds. This protects the intended fantasy that reputation can tip a marginal coercive situation without becoming a universal success bonus or requiring a global fear buff.
