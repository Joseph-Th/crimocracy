...
| --- | --- | --- |
| World | Organizations, characters, neighborhoods, businesses, institutional profiles, membership and supervision, player designation, daily payroll (all organization-owned liquid cash is eligible and available cash is shared evenly to the cent across active members; underpaid members with supervisors gain supervisor-directed resentment and the player organization receives a shortfall report); read-only per-district territory-influence summaries over enterprise records | `src/world/` via `world_system`, `payroll_execution`, `territory_influence` |
...
| Enterprises | Routine criminal enterprises (protection, gambling, alcohol distribution, bookmaking, loan sharking, fencing, speakeasy, labor racketeering) with authored economics, venue-function requirements version-pinned at establishment and resumption, one live racket of a kind per location, manager authority, scheduled cycles settling balanced; chronic net loss suspends at the authored threshold, counted since the latest resumption; suspended rackets may resume or be terminally retired, preserving history while releasing their kind/location slot for a fresh enterprise; active district casework adds a street-heat surcharge reported on appearance or change (an unchanged surcharge settles as routine cost) and rolls per-cycle vice attention that can open an originated inquiry on the racket itself with organization-held legal knowledge; financial reporting; daily delegated expansion for non-player organizations gated by police-fear posture and authored one-cycle operating-capital runway, with existing active rackets and earlier same-pass establishments reserving their claims against shared cash; viable expansion candidates must have positive current zero-variance production net, explicit led/other neighborhood scopes retain influence priority, specific business scopes follow, and an organization-wide Enterprise-function scope remains a broad fallback over all city neighborhoods and owned venues; economics ranks candidates within an authority tier, while deterministic minimum-cardinality support-business composition plus stable tie-breakers preserve replay | `src/enterprises/` via `enterprise_execution`, `autonomous_expansion`, and reporting modules |
| Legitimate economy | Business operating economies with ownership transfer/history, scheduled cycles, authored economics including the per-kind acquisition price, accounting information, comparative reporting, sabotage/arson disruption horizons with degraded earning power, chronic-loss suspension counted since the latest resumption; organizations buy independently owned businesses outright at that price by aggregating organization-owned accounted-funds accounts, with the ledger owning the exact deterministic debit allocation; purchase consideration leaves buyer-controlled spendable liquidity through the acquired business's non-liquid settlement counterparty rather than capitalizing its operating account; acquisitions open the first operating economy when none exists or reopen suspended books and surface as Notable financial reports for the buyer | `src/economy/` via `business_economy_system`, `business_acquisition`, and reporting modules |
...
| Legal institutions | Jurisdictions, patrol deployments, timed police response; investigations originated by operation exposure or enterprise vice attention (only originated cases decay cold; inactivity shelves even an identified at-large lead when modeled work stops, releasing the lead investigator, while a later incident sharing the shelf's subject matter resumes the existing file rather than opening a parallel case); evidence graphs; conflict-free single-seat staffing with investigator-held case-activity knowledge; sequential detective review attempts across each reviewable evidence source, with detention-cancelled work retryable after restaffing and completed inconclusive attempts not looped forever; witness interviews producing named testimony; autonomous evidence-threshold arrests across active LawEnforcement-owned investigations, counting only independent non-weak/non-questionable evidence not known inadmissible toward the two-item bar, while non-police LegalAuthority files do not exercise custody authority; autonomous custody is one-shot per case/person pair so unchanged evidence cannot create a release/re-arrest loop, and every detention ends at the authored maximum when no earlier canonical release occurs because charging, bail, and trial are outside the modeled foundation; custody atomically preempts the detainee's live operation, detective-work, investigation-lead, and prosecution assignments instead of allowing internal bookings to block arrest; detainee informant recruitment and disclosures; representation (direct retention and organization-policy automatic retention may aggregate sponsor-owned liquid accounts, with exact allocation only in the ledger; mandate-sourced automatic retention and explicitly delegated retention are constrained to the mandate's Legal scope, budget account, and current budget window; automatic-policy retention consults organization policy under the defendant's supervisor's mandate override and concludes after custody ends, while explicitly commanded retention persists); prosecution referral/review with replaceable current prosecutor staffing, defendant-specific referred evidence, and referral/resolution actor provenance, while the final terminal review releases the originating detention when no other prosecutor office still reviews that arrest | `src/legal/` named system modules; invariant validation split per aggregate under `src/core/invariants/legal/` |
