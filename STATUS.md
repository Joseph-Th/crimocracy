...
| Money laundering | Street-cash-to-accounted-funds conversion through owned cash-intensive fronts, authored fee split to the front's operating account, plausibility capacity capped at an authored fraction of the front's legitimate gross potential | `src/finance/` via `finance_system::validate_launder_funds` |
...
The current in-memory state schema version is 54.
