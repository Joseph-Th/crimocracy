...
- **Rival intelligence** — after a normal daily rival-expansion boundary, watch a known rival organization, select the first player-held Personnel observation with an Enterprise subject, and attach it to one direct follow-up watch. No hidden enterprise enumeration supplies targets. Failed/partial discovery records absence rather than inventing a target; the default-world regression requires the complete discovery/follow-up chain. `rival-intelligence-w<world>-p<policy>.json` retains exact summaries, quality, observation times, source identities, exposure and attached information; artifact IO failures propagate. Existing defector watches also surface their incidental racket sightings in the narrative, closing view and per-run `player_visible.known_rackets`; these are historical, bounded observations, never live portfolio totals. Production regressions cover bounded active-only discovery, partial/failed withholding, exact-subject authorization, snapshot freshness and persistence.
